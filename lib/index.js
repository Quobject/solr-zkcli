...
var startTime;
var waitForContainerToFinish = function (containerid, timeout) {
  ...
  if (!timeout) {
    startTime = moment();
    timeout = moment().add(15, 's');
  }
  //debug('timeout', timeout.format());

  var now = moment();

  //debug('now', now.format());
  var diff = now.valueOf() - startTime.valueOf();
  //debug('diff ms', diff);

  if (now.isSameOrBefore(timeout)) {
    return Promise.delay(500).then(function () {
      ...
      if (stillRunning) {
        return waitForContainerToFinish(containerid, timeout);
      }
